...
static ssize_t a1028_bootup_init(struct file *file, struct a1028img *img)
{
	...
	if (img->img_size > A1028_MAX_FW_SIZE) {
		pr_aud_err("%s: invalid a1028 image size %d\n", __func__,
			img->img_size);
		return -EINVAL;
	}
...
int a1028_filter_vp_cmd(int cmd, int mode)
{
	...
	pr_aud_info("%s: %x filtered = %x, a1028_NS_state %d, mode %d\n",\
...
int a1028_set_config(int newid, int mode)
{
	...
	if (rc < 0) {
		pr_aud_err("A1028 CMD block write error!\n");
...
static int
a1028_ioctl(struct inode *inode, struct file *file, unsigned int cmd,
	   unsigned long arg)
{
	...
	case A1028_SET_CONFIG:
		...
		if (rc < 0)
			pr_aud_err("%s: A1028_SET_CONFIG (%d) error %d!\n",
				__func__, pathid, rc);
		break;
	...
	case A1028_SET_PARAM:
		...
		pr_aud_info("%s: update a1028 i2c commands success.\n",\
			__func__);
		rc = 0;
		break;
#if ENABLE_DIAG_IOCTLS
	case A1028_SET_MIC_ONOFF:
		...
		if (rc < 0)
			pr_aud_err("%s: A1028_SET_MIC_ONOFF %d error %d!\n",
...
chk_gpio_micsel:
	...
	if (rc) {
		pr_aud_err("%s: a1028_device register failed\n", __func__);
		goto err_free_gpio_all;
	}
