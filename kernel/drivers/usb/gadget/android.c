...
static void bind_functions(struct android_dev *dev)
{
	...
	int	product_id, product_id_match;
    #if fcADD_ACCESSORY_VID
int	vendor_id;
    #endif
#endif
	...
	if (dev->match) {
		product_id_match = dev->match(product_id, intrsharing);
		printk(KERN_INFO "usb: product_id_match=0x%x\n", product_id_match);
		device_desc.idProduct = __constant_cpu_to_le16(product_id_match);
	} else
...
#else
	/* traverse all dynamic gadget who is willing to bind now */
...
static int android_bind(struct usb_composite_dev *cdev)
{
	...
	int			gcnum, id, product_id, ret, product_id_match;
...
int android_switch_function(unsigned func)
{
	...
	list_for_each_entry(f, &android_config_driver.functions, list) {
		...
		if (!strcmp(f->name, "usb_mass_storage")) {
			...
			!strcmp(f->name, "ether")) {
			if (func & (1 << USB_FUNCTION_RNDIS_IPT))
				intrsharing = 0;
			else
				intrsharing = 1;
			if (f->hidden) {
