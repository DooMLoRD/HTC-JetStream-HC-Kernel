...
static struct platform_driver *p_sdio_platform_driver[N_PORTS] = {0};
static int gsdio_init_flag[N_PORTS] = {0};
...
static int gsdio_ch_probe(struct platform_device *dev)
{
	...
	for (i = 0; i < n_ports; i++) {
		...
		pr_info("%s: sdio_ch_name:%s dev_name:%s\n", __func__,
		...
		struct sdio_port_info *pi)
{
	...
	p_sdio_platform_driver[portno] = pdriver;
	if (gsdio_init_flag[portno] == 1 && !port->sdio_probe)
...
{
	struct platform_driver *p = p_sdio_platform_driver[port];
	struct gsdio_port *gp = ports[port].port;

	/* Only for DUN currently
	 */
	pr_info("%s: port: %d, ebl: %d, sdio_probe: %d\n", __func__,
			port, ebl_state, gp?gp->sdio_probe:0);
	if (port == 0) {
		if (ebl_state == 1) {
			if (p && gp && !gp->sdio_probe)
				platform_driver_register(p);
			else
				gsdio_init_flag[port] = 1;
		} else {
			if (p && gp && gp->sdio_probe) {

				/* ToDo, unregister the driver
				 * platform_driver_unregister(p);
				 */
			}
		}
	}
