/* linux/arch/arm/mach-msm/board-verdi-lte-wifi.c
*/
...
int verdi_lte_wifi_power(int on);
int verdi_lte_wifi_reset(int on);
int verdi_lte_wifi_set_carddetect(int on);
int verdi_lte_wifi_get_mac_addr(unsigned char *buf);
...
static void *verdi_lte_wifi_mem_prealloc(int section, unsigned long size)
{
...
int __init verdi_lte_init_wifi_mem(void)
{
	int i;
...
static struct resource verdi_lte_wifi_resources[] = {
	[0] = {
		.name		= "bcm4329_wlan_irq",
		.start		= MSM_GPIO_TO_INT(VERDI_LTE_GPIO_WIFI_IRQ),
		.end		= MSM_GPIO_TO_INT(VERDI_LTE_GPIO_WIFI_IRQ),
#ifdef HW_OOB
...
static struct wifi_platform_data verdi_lte_wifi_control = {
	.set_power      = verdi_lte_wifi_power,
	.set_reset      = verdi_lte_wifi_reset,
	.set_carddetect = verdi_lte_wifi_set_carddetect,
	.mem_prealloc   = verdi_lte_wifi_mem_prealloc,
	.get_mac_addr	= verdi_lte_wifi_get_mac_addr,
	.dot11n_enable  = 1,
};

static struct platform_device verdi_lte_wifi_device = {
	.name           = "bcm4329_wlan",
	.id             = 1,
	.num_resources  = ARRAY_SIZE(verdi_lte_wifi_resources),
	.resource       = verdi_lte_wifi_resources,
	.dev            = {
		.platform_data = &verdi_lte_wifi_control,
	},
};

extern unsigned char *get_wifi_nvs_ram(void);

static unsigned verdi_lte_wifi_update_nvs(char *str)
{
...
int verdi_lte_wifi_get_mac_addr(unsigned char *buf)
{
	...
	printk("verdi_lte_wifi_get_mac_addr = %02x %02x %02x %02x %02x %02x \n",
...
#else
	verdi_lte_wifi_update_nvs("sd_oobonly=1\n");
#endif
	verdi_lte_wifi_update_nvs("btc_params80=0\n");
	verdi_lte_wifi_update_nvs("btc_params6=30\n");
	verdi_lte_init_wifi_mem();
	ret = platform_device_register(&verdi_lte_wifi_device);
	return ret;
}
