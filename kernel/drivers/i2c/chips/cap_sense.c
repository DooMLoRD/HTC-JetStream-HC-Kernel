...
#ifdef CONFIG_MSM8X60_CAPSENSE
#include <mach/oem_rapi_client.h>
#define CAPSENSE_CLIENT_EVENT_WRITE 2001
#define MDM_9K_DELAY HZ*25
#endif
...
static struct workqueue_struct *csa_reconfigure_th_wq;
struct delayed_work csa_reconfigure_th_delayed_work;
#endif
...
struct capsense_platform_data *this_pdata;
...
static irqreturn_t cap_sense_irq_handler(int irq, void *data)
{
	...
	queue_delayed_work(capsense_wq, &cd->work, 0);
	return IRQ_HANDLED;
}
...
static void capsense_work_func(struct work_struct *work)
{
	struct capsense_data *cs = container_of(
			work, struct capsense_data, work.work);
#ifdef CONFIG_MSM8X60_CAPSENSE
	...
	if (active != cs->is_actived) {
		...
		if (IS_ERR(rpc_client)) {
			SAR_LOGE("%s: couldn't open oem rapi client\n", __func__);
			queue_delayed_work(capsense_wq, &cs->work, HZ);
			return;
		} else {
			rpc_input_data = (char)active;
			arg.event = CAPSENSE_CLIENT_EVENT_WRITE;
			arg.cb_func = NULL;
			arg.handle = (void *)0;
			arg.in_len = 2;
			arg.input = &rpc_input_data;
			arg.out_len_valid = 0;
			arg.output_valid = 0;
			arg.output_size = 0;
			ret.out_len = NULL;
			...
			if (err)
				SAR_LOGE("%s: Send data from modem failed: %d\n", __func__, err);
			else
...
static void capsense_sleep_func(struct work_struct *work)
{
	...
	switch (mode) {
	case KEEP_AWAKE:
		if (this_pdata->gpio_init)
			this_pdata->gpio_init(1);
		gpio_direction_output(cs->intr_gpio, 0);
		msleep(1);
		gpio_direction_input(cs->intr_gpio);
		msleep(200);
		csa_set_kadc();
		queue_delayed_work(capsense_wq, &cs->work, 0);
		break;

	case DEEP_SLEEP:
		disable_irq(cs->intr_irq);
		if (cancel_delayed_work_sync(&cs->work))
			enable_irq(cs->intr_irq);
...
static ssize_t csa_test_store(struct device *dev,
				 struct device_attribute *attr,
				const char *buf, size_t count)
{
#ifdef CONFIG_MSM8X60_CAPSENSE
...
static DEVICE_ATTR(test, 0664, NULL, csa_test_store);

/* dev_attr_reset */
static ssize_t csa_reset_store(struct device *dev,
					 struct device_attribute *attr,
					 const char *buf, size_t count)
{
	SAR_LOGI("%s\n", __func__);
	if (this_pdata->chip_reset)
		this_pdata->chip_reset();

	return count;
}
static DEVICE_ATTR(reset, 0664, NULL, csa_reset_store);

#ifdef CONFIG_HTC_PUI_I2C_SAR_RECOVERY
static void csa_reconfigure_th_work_func(struct work_struct *work)
{
	SAR_LOGI("%s\n", __func__);
	csa_set_kadc();
}

void capsense_reconfigure_th(void)
{
	cancel_delayed_work_sync(&csa_reconfigure_th_delayed_work);
	if (csa_reconfigure_th_wq != NULL)
		queue_delayed_work(csa_reconfigure_th_wq, &csa_reconfigure_th_delayed_work,
				   msecs_to_jiffies(200));
	else
		SAR_LOGI("%s, csa_reconfigure_th_wq is NULL\n", __func__);
}
#endif
					...
					const struct i2c_device_id *id)
{
	...
	if (capsense_wq == NULL)
		goto err_create_capsense_wq;
	INIT_DELAYED_WORK(&cs->work, capsense_work_func);
	INIT_DELAYED_WORK(&cs->sleep_work, capsense_sleep_func);
#ifdef CONFIG_HTC_PUI_I2C_SAR_RECOVERY
	INIT_DELAYED_WORK(&csa_reconfigure_th_delayed_work, csa_reconfigure_th_work_func);
	csa_reconfigure_th_wq = create_singlethread_workqueue("csa_reconfigure_th_wq");
	if (csa_reconfigure_th_wq == NULL)
		goto err_create_csa_reconfigure_th_wq;
#endif
	...
	if (pdata->intr) {
		...
		err = request_any_context_irq(cs->intr_irq, cap_sense_irq_handler,
...
#ifdef CONFIG_MSM8X60_CAPSENSE
	queue_delayed_work(capsense_wq, &cs->work, MDM_9K_DELAY);
#else
	queue_delayed_work(capsense_wq, &cs->work, 0);
#endif
	...
	err = device_create_file(cs->capsense_dev, &dev_attr_test);
	if (err)
		SAR_LOGE("%s:fail to create attr_test, err=%d\n", __func__, err);
	err = device_create_file(cs->capsense_dev, &dev_attr_reset);
	if (err)
		SAR_LOGE("%s:fail to create attr_reset, err=%d\n", __func__, err);
...
request_gpio_failed:
#ifdef CONFIG_HTC_PUI_I2C_SAR_RECOVERY
	destroy_workqueue(csa_reconfigure_th_wq);
err_create_csa_reconfigure_th_wq:
#endif
	destroy_workqueue(capsense_wq);
err_create_capsense_wq:
