...
/* HTC_CSP_START */
#if defined(CSCAN)
static int presist_cnt;
static int old_rxintcnt;
static int old_rxdfrmmcast;
static int old_rxmfrmmcast;
static int old_rxmfrmocast;
static int cur_rxintcnt;
static int cur_rxdfrmmcast;
static int cur_rxmfrmmcast;
static int cur_rxmfrmocast;
static int ap_found;
static int noscanrslt;
#endif
/* HTC_CSP_END */
...
wl_iw_counters(struct net_device *dev)
{
	...
	if (!counter_buf) {
		printk(KERN_ERR "no enough mem!\n");
		return -ENOMEM;
	}

	pbuf = counter_buf;
	dev_iw_iovar_getbuf(dev, "counters", "", 0, pbuf, 1536);
...
/* HTC_CSP_START */
	if ((noscanrslt == 1)
	&& (!strncmp(flag, "HANG", 4))) {
		WL_ERROR(("skip sent HAND EVENT "));
	} else {
		WL_ERROR(("sen event to supp %s", extra));
...
/* Htc_CSP_START */
    ...
    if (counter == 0) {
		wl_iw_counters(dev);
		dev_wlc_intvar_get(dev, "rxintcnt", &cur_rxintcnt);
		if (((presist_cnt == 0) && ap_found)
		|| (presist_cnt == 1)) {
			presist_cnt++;
			WL_ERROR(("[%s], rxintcnt = %d, presist_cnt = %d\n", __func__, cur_rxintcnt, presist_cnt));
			if (ap_found)
				error = -EINVAL;
			ap_found = 0;
		}
...
/* HTC_CSP_START */
		noscanrslt = 0; /* reset no scan result flag*/
/* HTC_CSP_END */
        dhd_os_start_unlock(iw->pub);
...
#ifdef CSCAN
/* HTC_CSP_START */
	memset(&cnt_old, 0, sizeof(wl_cnt_t));
/* HTC_CSP_END */
