...
static int snddev_icodec_open_rx(struct snddev_icodec_state *icodec)
{
	...
	if (support_adie) {
		if ((!support_aic3254) ||
			(support_aic3254 && (!strcmp(icodec->data->name, \
			"usb_headset_stereo_rx")))) {
			/* For supporting aic3254 case,
			 * turn on adie for usb audio only.
			 */

			/* Configure ADIE */
			trc = adie_codec_open(icodec->data->profile, \
				&icodec->adie_path);
			if (IS_ERR_VALUE(trc))
			...
			/* OSR default to 256, can be changed for
			 * power optimization. If OSR is to be changed,
			 * need clock API for setting the divider.
			 */
		}
	}
	...
	if (support_adie) {
		if ((!support_aic3254) ||
			...
			"usb_headset_stereo_rx")))) {
			/* For supporting aic3254 case,
			 * only turn on adie for usb audio.
			 */
...
static int snddev_icodec_open_tx(struct snddev_icodec_state *icodec)
{
	...
	if (support_adie && !support_aic3254) {
		/* Enable ADIE */
	...
	if (icodec->adie_path && support_adie && !support_aic3254) {
		adie_codec_proceed_stage(icodec->adie_path,
					ADIE_CODEC_DIGITAL_READY);
		adie_codec_proceed_stage(icodec->adie_path,
					ADIE_CODEC_DIGITAL_ANALOG_READY);
