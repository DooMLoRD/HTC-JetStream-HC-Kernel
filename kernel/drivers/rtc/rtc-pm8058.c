...
#ifdef CONFIG_RTC_PM8058_STOP_ENABLE
static struct pm8058_chip *rtc_pm_chip;
#endif
...
#ifdef CONFIG_RTC_PM8058_STOP_ENABLE
void pm8058_rtc_alarm_disable(void)
{
	int rc;
	u8 reg;

	rc = pm8058_read(rtc_pm_chip, PM8058_RTC_CTRL, &reg, 1);
	if (rc < 0) {
		pr_err("%s: PM8058 read failed\n", __func__);
		return;
	}
	...
	reg &= ~PM8058_RTC_ENABLE;
	rc = pm8058_write(rtc_pm_chip, PM8058_RTC_CTRL, &reg, 1);
	if (rc < 0)
		pr_err("%s: PM8058 write failed\n", __func__);
