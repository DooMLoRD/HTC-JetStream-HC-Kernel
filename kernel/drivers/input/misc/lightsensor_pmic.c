...
#define DLS(x...) printk(KERN_DEBUG "[LS][CM3602 PMIC] " x)
#define ILS(x...) printk(KERN_INFO "[LS][CM3602 PMIC] " x)
#define ELS(x...) printk(KERN_ERR "[LS][CM3602 PMIC ERROR] " x)
...
static struct smd_ls_info *ls_smd_info;
...
static void set_ls_kvalue_smd(struct smd_ls_info *li)
{
	if (!li) {
		ELS("%s: smd_ls_info is empty\n", __func__);
		return;
	}
...
static uint32_t lightsensor_read_adc_by_pmic(void)
{
	/* struct smd_ls_info *pinfo = ls_smd_info; */
	uint32_t ls_adc = -1;/*adc = 1250 * mean_adc / mean_vref*/

	int mean_vref, mean_adc;
	int vref[1], adc[1];
	int i, rc = 0;
	int result;
	...
	rc = pm8058_htc_config_mpp_and_adc_read(
				...
				ls_smd_info->mpp_data->lightsensor_mpp,
				ls_smd_info->mpp_data->lightsensor_amux);
	if (rc) {
		ELS("Get adc failed.\n");
		ls_adc = 0xFFFFF;
		return ls_adc;
	...
	ls_adc = 1250 * mean_adc / mean_vref;
	/*DLS("%s: result = %d\n", __func__, ls_adc);*/
...
static int get_ls_adc_level(uint8_t *data)
{
	...
	adc_value = lightsensor_read_adc_by_pmic();
	adc_value = adc_value *(0xFFFF /li->pdata->m_voltage);
...
static void report_lightseneor_data_smd(void)
{
	uint8_t data[3];
	int ret;
	struct smd_ls_info *li = ls_smd_info;
...
static int lightsensor_probe(struct platform_device *pdev)
{
	...
	ret = ls_power(1);
	if (ret < 0) {
		ELS("%s: Fail to power on the lightsensor\n", __func__);
		goto err_ls_power;
	}
