...
drop:
				...
				if ((driver->qxdm2sd_drop) && (driver->logging_mode == USB_MODE)) {
					...
					print_hex_dump(KERN_DEBUG, "Drop Packet Data"
						" from 9K(first 16 bytes)", 16, 1, DUMP_PREFIX_ADDRESS, buf, 16, 1);					
				...
				if (diag9k_debug_mask) {
					...
					default:
						print_hex_dump(KERN_DEBUG, "Read Packet Data"
						" from 9K ", 16, 1, DUMP_PREFIX_ADDRESS, buf, r, 1);
...
{
	if (diag9k_debug_mask)
		DIAG_INFO("%s event=%d(1:read 2:write) buf:%d\n", __func__, event, driver->in_busy_sdio);
