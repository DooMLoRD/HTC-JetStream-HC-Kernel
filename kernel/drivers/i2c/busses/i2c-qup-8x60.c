...
static void
QUP_I2C_PUI_SAR_workaround(struct qup_i2c_dev *dev)
{
	dev_err(dev->dev, "[QUP_I2C_PUI_SAR_workaround], nr:%d\n", dev->adapter.nr);
	dev->pdata->chip_reset();
}

static void
QUP_I2C_SHOW_SCL_SDA(struct qup_i2c_dev *dev)
{
	int gpio_clk = 0, gpio_dat = 0;

	if (qup_i2c_config_gpio(dev, &gpio_clk, &gpio_dat))
		return;
	dev_info(dev->dev, "QUP_I2C_SHOW_SCL_SDA, dev->adapter.nr: %d, gpio_clk: %d, gpio_dat: %d\n",
		 dev->adapter.nr, gpio_clk, gpio_dat);
	dev_info(dev->dev, "QUP_I2C_SHOW_SCL_SDA, i2c_scl: %d, i2c_sda: %d\n",
...
qup_i2c_xfer(struct i2c_adapter *adap, struct i2c_msg msgs[], int num)
{
	...
	while (rem) {
		...
		if (qup_i2c_poll_state(dev, QUP_I2C_MAST_GEN) != 0) {
			ret = -EIO;
			dev_err(dev->dev, "qup_i2c_xfer: qup_i2c_poll_state() ERR!\n");
			goto out_err;
		}
		...
		if (err < 0) {
			ret = err;
			dev_err(dev->dev, "qup_i2c_xfer: qup_update_state(RUN) ERR!\n");
			goto out_err;
		}
...
#ifdef CONFIG_HTC_PUI_I2C_SAR_RECOVERY
				dev_err(dev->dev, "[QUP I2C Err] PUI_I2C_SAR_RECOVERY, dev->adapter.nr == %d\n", dev->adapter.nr);
				if ((dev->adapter.nr == 4) && (dev->pdata->chip_reset)) {
					dev_err(dev->dev, "[QUP I2C Err] PUI_I2C_SAR_RECOVERY: QUP status flags :0x%x\n", istatus);
					QUP_I2C_SHOW_SCL_SDA(dev);
					QUP_I2C_PUI_SAR_workaround(dev);
				}
#endif
		...
		if (dev->cnt == 0) {
			...
			if (rem) {
				...
				if (err < 0) {
					ret = err;
					dev_err(dev->dev, "qup_i2c_xfer: qup_update_state(RESET) ERR!\n");
					goto out_err;
				}
			}
		}
...
#ifdef CONFIG_HTC_PUI_I2C_SAR_RECOVERY
	if (!(pdata->chip_reset)) {
		dev_err(&pdev->dev, "[QUP I2C Err] chip_reset function not initialized\n");
	}
#endif
